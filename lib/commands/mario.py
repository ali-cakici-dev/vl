...
def mario(proto, payload):
    """ Play "Super Mario Bros" main theme on the buzzer """
    import intellireader.misc.buzzer_pb2 as buzzer_pb2

    request = commands_pb2.Miscellaneous()
    cmd = request.make_sound

    for (note, temp) in zip(MELODY, TEMPO):
        sound = cmd.melody.add()
        sound.frequency_hz = int(note * 0.20)
        sound.duration_ms = 900 // temp
        sound.silence_duration_ms = int(sound.duration_ms * 1.30)
